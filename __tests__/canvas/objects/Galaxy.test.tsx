...
describe('Galaxy', () => {
  it('renders without throwing when parked at z=-220 (progress < 0.75)', () => {
    expect(() =>
      render(<Galaxy scrollProgress={{ current: 0 }} />),
    ).not.toThrow();
  });

  it('renders without throwing during approach (progress 0.75–0.95)', () => {
    expect(() =>
      render(<Galaxy scrollProgress={{ current: 0.85 }} />),
    ).not.toThrow();
  });

  it('renders without throwing at full approach (progress = 1.0)', () => {
    expect(() =>
      render(<Galaxy scrollProgress={{ current: 1.0 }} />),
    ).not.toThrow();
  });
...
function galaxyZ(progress: number): number {
  ...
  if (progress < 0.95)
    return lerp(-220, -100, smoothstep((progress - 0.75) / 0.2));
