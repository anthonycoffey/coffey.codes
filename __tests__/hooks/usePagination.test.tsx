...
describe('usePagination', () => {
  ...
  it('reads initial page from ?page search param', () => {
    vi.mocked(useSearchParams).mockReturnValue(
      new ReadonlyURLSearchParams(new URLSearchParams('page=3')),
    );
    const { result } = renderHook(() => usePagination(5));
  ...
  it('isFirstPage is false on page 2+', () => {
    vi.mocked(useSearchParams).mockReturnValue(
      new ReadonlyURLSearchParams(new URLSearchParams('page=2')),
    );
    const { result } = renderHook(() => usePagination(5));
  ...
  it('isLastPage is true on the last page', () => {
    vi.mocked(useSearchParams).mockReturnValue(
      new ReadonlyURLSearchParams(new URLSearchParams('page=5')),
    );
    const { result } = renderHook(() => usePagination(5));
