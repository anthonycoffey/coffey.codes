...
summary: 'Ever needed to trace exactly what happened to a rogue file? Well, there's a Git command for that!'
tags: Git, Version Control, File History, Command Line, Tracking Changes, Debug
...
<Callout type="tip">
  If you have a large number of commits and you want to narrow down the search,
  you can add additional options to the git log command, such as `--author` to
  filter by the commit author, or `--before` and `--after` to specify a date
  range.
</Callout>
