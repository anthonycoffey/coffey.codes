...
Historically, to run E2E tests in CI, I would build the Next.js app on the GitHub runner, spin it up locally, and run Playwright against it. This was painfully slow and fundamentally flawed—it didn't test the actual edge network, serverless functions, or caching layer that production uses.

Then came the realization: **Vercel is already building a Preview Deployment for every PR.** Why build it twice?

By moving our E2E tests to run against the Vercel Preview, I cut out the redundant build step entirely. This saved ~2–3 minutes per PR _and_ made the test run infinitely more trustworthy because it was hitting a real edge runtime.
...
To fix this, I created a **Protection Bypass for Automation** token in Vercel (Settings → Deployment Protection), stored it as a GitHub secret (`VERCEL_AUTOMATION_BYPASS_SECRET`), and passed it as an environment variable to Playwright along with the live target URL.
