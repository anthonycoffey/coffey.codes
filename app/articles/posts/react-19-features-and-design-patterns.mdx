...
It's important to note that Server Components do not use a specific directive like "use server". Instead, the "use server" directive is intended for Server Functions. For further details, refer to the [Directives documentation](https://react.dev/reference/rsc/directives).
