...
const STAR_POSITIONS = (() => {
  ...
  for (let i = 0; i < PARTICLE_COUNT; i++) {
    ...
    arr[i * 3 + 2] = STAR_SPREAD_Z_MIN + Math.random() * (STAR_SPREAD_Z_MAX - STAR_SPREAD_Z_MIN)
  }
  return arr
})()
...
function ParticleSystem({ scrollProgress }: ParticleSystemProps) {
  ...
  useFrame(({ clock }) => {
    ...
    const formFactor = progress < 0.15
      ? 1 - smoothstep(progress / 0.15)
      : 0

    // ── Lerp positions toward target ──────────────────────────
    const buf = currentPositions.current
    for (let i = 0; i < PARTICLE_COUNT * 3; i++) {
      const target = MERKABA_POSITIONS[i] * formFactor
  ...
  return (
    ...
    >
      ...
      >
        ...
        <directionalLight position={[10, 20, 5]} intensity={1.2} color="#ffe8cc" />
