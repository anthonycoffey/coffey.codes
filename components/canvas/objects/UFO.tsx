...
export default function UFO({ scrollProgress }: UFOProps) {
  ...
  useFrame(({ clock }) => {
    ...
    if (progress < 0.38) {
      ...
      const y = 1.2 + Math.sin(t * 0.8) * 0.25
      ...
      const y = lerp(1.2, 2.5, flyT)   // slight upward drift — flies "over"
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Lights */}
      <pointLight position={[0.5, -0.1, 0.5]}   color="#00e5ff" intensity={2} distance={5} />
      <pointLight position={[-0.5, -0.1, -0.5]}  color="#00ff88" intensity={2} distance={5} />
      <pointLight position={[0, -0.2, 0]}         color="#8b5cf6" intensity={1} distance={3} />
    </group>
  )
}
