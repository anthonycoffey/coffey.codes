...
`components/RelatedPosts.tsx` renders at the foot of every article (server component, added in SPEC-030 WS3). It surfaces up to three related posts ranked by shared-tag count → same-category tiebreak → recency. This gives every article a structural internal-link path to its closest siblings without manual curation. Pure ranking function (`selectRelatedPosts(current, candidates, limit)`) is exported separately for unit tests; tag matching is case-insensitive and trimmed so frontmatter casing differences don't break relatedness.

Heading-level anchor links inside articles are auto-generated by `components/mdx.tsx` `createHeading` (a slugified `id` plus a clickable anchor on every h2-h6). The same mechanism creates the SERP candidates Google indexes as separate URLs — see `docs/strategy/content-disposition.md` row #4 (react-19) for a worked example of when this becomes self-cannibalization.

### `dateModified` policy

`dateModified` reflects substantive edits — content changes AND SERP-affecting metadata changes (title, meta description). It does **not** reflect whitespace or typo fixes. The author updates the MDX frontmatter `updated:` field manually when the change is meaningful; `BlogPosting.dateModified` and the sitemap `lastModified` both read `metadata.updated ?? metadata.publishedAt` (the sitemap wiring was added in SPEC-030 WS4 — refreshed articles now signal the update to crawlers, where previously only `publishedAt` was sent).

Title and meta-description rewrites count as substantive because they change what Google shows in the SERP snippet, which directly affects CTR. SPEC-030 WS1 set `updated:` on four articles whose only edit was the frontmatter title/summary — that's the canonical example of the policy.

Audit docs in `docs/strategy/seo-audit-YYYY-QN.md` track whether crawl recency is improving as a result. The 2026-05-23 update of `updated:` on the SPEC-030 WS1 articles will be visible in the 2026-08-10 quarterly audit.
