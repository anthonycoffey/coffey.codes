...
Iteration 4 (shipped) — drop the scroll gate entirely; share the orbit center with the rock. Feedback: the orbit should be visible at all scroll positions, including `0.00`, so the homepage feels alive from the moment the canvas mounts. The activation latch, `HIDDEN_POS`, and scroll-dependency were all removed; position is now driven purely from `clock.getElapsedTime()`. Separately, the rock's orbit was found to not share the satellite's planetary center, which read as unmotivated; this was corrected so both bodies orbit the _same_ center, in the _same_ plane, at the _same_ radius, with the rock starting at `satInitial + π` (opposite side) and revolving at `-0.5` rad/s (reverse direction, faster than the satellite's `+0.3`). The two now drift past each other unpredictably as they share the orbit. Satellite attitude is also relaxed — a slow Y-axis spin (`0.1` rad/s) plus sinusoidal pitch (`±8°`) and roll (`±5°`) at distinct rates — producing a "drifting attitude" rather than a full tumble so the silhouette stays legible.
