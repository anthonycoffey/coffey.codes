...
- 2026-05-23. SPEC-030 (technical SEO sprint) shipped. Calendar implications:
  - **Slot 2 (Expo Location deep-dive)** is reinforced, not changed. Live `seo_striking_distance` pull on 2026-05-22 confirms 8 of the top 10 striking-distance entries are still Expo Location function-name long-tails (`requestForegroundPermissionsAsync`, `hasservicesenabledasync`, `getforegroundpermissionsasync`, etc.) at position 8-10 with 0 clicks. The parent article's WS1 title/summary rewrite addresses the head-query CTR gap; the function-name cluster still needs Slot 2.
  - **Slot 4 (Firebase App Hosting env-var patterns)** is also reinforced. `apphosting.yaml` at pos 9.0 / 59 impr, `firebase app hosting apphosting.yaml` at pos 9.0 / 24 impr, `next_public_firebase_api_key` at pos 9.5 / 26 impr — all 0 clicks. WS1 added `apphosting.yaml` verbatim to the parent article's title; Slot 4 still owns the function-by-function deep-dive.
  - **Slot 5 (Slow Android emulator refresh)** target queries remain at the same striking distance. WS1 rewrote title/summary as a metadata interim; this **does not** substitute for the full refresh — the article body still needs the current-Android-Studio diagnostic flow update per the slot brief. Target ship 2026-12-31 unchanged.
  - **Slot 1 (Vibe-coding refresh)** unchanged. WS1 deliberately did not touch the vibe-coding article — its 2.73% CTR is healthy, no metadata gap to close.
  - **New editorial item (uncalendared)**: react-19 article self-cannibalization via TOC anchor fragments. Not metadata-fixable per SPEC-030 WS6 finding. See `content-disposition.md` row #4 — the next refresh of this article should restructure or remove the visible TOC list to reduce anchor-fragment SERP candidates.
